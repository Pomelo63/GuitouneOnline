...
func _process(_delta):
	...
	if is_chatting:
		if Input.is_action_just_pressed("ui_accept"):
			if dialog_box and dialog_box.handle_input_accept():
				# On a juste affiché tout le texte, on ne ferme pas
				return
			else:
				# Le texte était déjà affiché, on ferme
				hide_dialogue()
		return
...
func start_dialogue():
	...
	show_dialog("Nom du PNJ", "Bonjour, aventurier. J’ai une quête pour toi.")
